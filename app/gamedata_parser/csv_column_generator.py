...
from app.db import models_generated
    ...
    def generate(self, log_stream: io.StringIO) -> str:
        ...
        # Foreign keys
            ...
            if not self.__model_file_exists(self.foreign_model):
                self.__add_to_json_config(self.foreign_model, log_stream)

        return model_field

    def __add_to_json_config(self, model_name: str, log_stream: io.StringIO):
        ...
        with open(config.filepath_gamedata_parser_json, "r+") as f:
            ...
            if datatype_str not in lst:
                print(f"Adding {model_name}.csv to gamedata_parser.json.", file=log_stream)
