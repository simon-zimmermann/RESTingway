...
class CSVModelGenerator():
    ...
    def generate(self):  # TODO retrurn a report
        model_fileds = ""
        schema_fields = ""
        import_list = []
        ...
        for i in range(len(self.csv_colnames)):
            ...
            (model_field, schema_field, foreign_model) = self.generate_column(csv_colname, csv_datatype)
            ...
            if foreign_model != "" and foreign_model not in import_list:
                ...
                if not self.model_file_exists(foreign_model):
                    self.add_to_parsingway_json(foreign_model)

        generated_code = self.generate_model_code(model_fileds, schema_fields, import_list)
        self.save_generated_code(generated_code)

    def generate_column(self, csv_colname: str, csv_datatype: str):
        py_colname = CSVUtil.convert_colnames(csv_colname)
        py_datatype = CSVUtil.convert_datatype(csv_datatype)
        model_field = ""
        schema_field = ""
        foreign_model = ""
        ...
        # Handle foreign keys
            ...
            foreign_table = CSVUtil.to_table_name(csv_datatype)
            ...
            schema_field = f"        {foreign_table}: {foreign_model}.{foreign_model}.Schema\n"
    ...
    def add_to_parsingway_json(self, model_name: str):
    ...
    def generate_model_code(self, model_fileds: str, schema_fields: str, import_list: list[str]):
...
class {self.model_name}(TableBase):
    ...
    __tablename__ = "{CSVUtil.to_table_name(self.model_name)}"
...
{model_fileds}
    ...
    def save_generated_code(self, generated_code: str):
