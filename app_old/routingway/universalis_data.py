...
@router.get("/universalis/averages")
def universalis_averages():
    itemlist: list[Item] = []
    for method in GatheringTypes:
        itemlist += crud_gathering.get_items_gatherable(method)
    itemlist = filter(lambda x: not x.is_untradable, itemlist)
    data = crud_universalis.get_list_averages(itemlist)
    data.sort(key=lambda x: x["minimum"], reverse=True)
    return data
